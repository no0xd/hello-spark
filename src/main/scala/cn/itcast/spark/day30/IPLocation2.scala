...
object IPLocation2 {
  ...
  val data2MySQL = (iterator: Iterator[(String, Int)]) => {
    ...
    try {
      conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/bigdata", "root", "asdf")
  ...
  def main(args: Array[String]) {
    ...
    val ipRulesRdd = sc.textFile("F:\\video\\传智播客三期视频\\day30\\计算IP地址归属地\\ip.txt").map(line => {
    ...
    //加载要处理的数据
    val ipsRDD = sc.textFile("F:\\video\\传智播客三期视频\\day30\\计算IP地址归属地\\20090121000132.394251.http.format").map(line => {
      val fields = line.split("\\|")
      fields(1)
    })
